	...
	for (size_t i = 0; i < blk->code.len; ++i) {
		...
		case ir_op_call: {
			...
			if (ts <= 8) {
				...
				ret->i = asm_ret_regs[0];
			} else if (ts <= 16) {
				ret->tag = ADDR_WIDE;
				ret->wide[0] = asm_ret_regs[0];
				ret->wide[1] = asm_ret_regs[1];
			} else {
			...
			while (argc--) {
				...
				if (ts <= 8 && arg_reg < ASM_ARG_REG_COUNT) {
					...
					arg->i = asm_arg_regs[arg_reg++];
				} else if (ts <= 16 && arg_reg < ASM_ARG_REG_COUNT - 1) {
					arg->tag = ADDR_WIDE_ARG;
					arg->wide[0] = asm_arg_regs[arg_reg++];
					arg->wide[1] = asm_arg_regs[arg_reg++];
				} else {
	...
	case ir_op_ret: {
		if (term->args.len == 1) {
			...
			if (addr->tag == ADDR_STACK) {
				...
				addr->i = asm_ret_regs[0];
			} else if (addr->tag == ADDR_REGISTER) {
				assert(addr->i == asm_ret_regs[0]);
			} else if (addr->tag == ADDR_TEMP_WIDE) {
				addr->tag = ADDR_WIDE;
				addr->wide[0] = asm_ret_regs[0];
				addr->wide[1] = asm_ret_regs[1];
			} else if (addr->tag == ADDR_WIDE) {
				if ((enum asm_register)addr->wide[0] == asm_ret_regs[0]
					&& (enum asm_register)addr->wide[1] == asm_ret_regs[1]) {
					/* do nothing */
				} else {
					FAILWITH("TODO:\n"
							 "addr->wide[0] = %s\n"
							 "addr->wide[1] = %s",
							 asm_reg_q_name[addr->wide[0]],
							 asm_reg_q_name[addr->wide[1]]);
				}
			} else {
...
asm_create_context(struct ir_proc *proc, struct da_pointers *blocks, struct asm_context *ctx)
{
	...
	for (size_t arg_num = 0; arg_num < entry->args.len; ++arg_num) {
		...
		if (ts <= 8 && arg_reg < ASM_ARG_REG_COUNT) {
			...
			ctx->vars[x].i = asm_arg_regs[arg_reg++];
		} else if (ts <= 16 && arg_reg < ASM_ARG_REG_COUNT - 1) {
			ctx->vars[x].tag = ADDR_WIDE;
			ctx->vars[x].wide[0] = asm_arg_regs[arg_reg++];
			ctx->vars[x].wide[1] = asm_arg_regs[arg_reg++];
		} else {
...
asm_emit_move_to_callee_save(struct asm_address *addr, struct asm_procedure *code)
{
	...
	append_line(&code->body, fmt_str(
					"\tmovq %s, %s\n",
					asm_reg_q_name[addr->i],
					asm_reg_q_name[reg]));
	asm_unassign_register(ctx, addr->i);
						  ...
						  struct asm_context *ctx, struct ir_toplevel *tl)
{
	...
	switch (src->tag) {
	case ADDR_REGISTER:
		asm_unassign_register(ctx, src->i);
		s = asm_reg_name(src->i, type);
		break;
	...
	case ADDR_IMM_INT:
		snprintf(buf, sizeof(buf), "$%ld", src->i);
		break;
	case ADDR_IMM_FLOAT:  FAILWITH("TODO: ADDR_IMM_FLOAT");  break;
	case ADDR_STACK:
		snprintf(buf, sizeof(buf), "%ld(%%rbp)", src->i);
		break;
	case ADDR_STACK_LOAD:
		snprintf(buf, sizeof(buf), "%d(%%rbp)", src->stack[0] + src->stack[1]);
		break;
	case ADDR_SYMBOL:
		/* snprintf(buf, sizeof(buf), "\"%s\"", tl->elems[src->i].hddr.link); */
		/* break; */
		return ir_object_link_name(tl, &tl->elems[src->i]);
...
asm_unassign_blk_arg_register(struct asm_context *ctx, int blk_arg)
{
	...
	struct asm_address *a = &ctx->vars[b->i];
	if (a->tag == ADDR_REGISTER || a->tag == ADDR_ARGUMENT) {
		asm_unassign_register(ctx, a->i);
	} else if (a->tag == ADDR_BLK_ARG) {
		asm_unassign_blk_arg_register(ctx, b->i);
	} else if (a->tag == ADDR_WIDE || a->tag == ADDR_WIDE_ARG) {
		asm_unassign_register(ctx, a->wide[0]);
		asm_unassign_register(ctx, a->wide[1]);
...
emit_op_mov_ADDR_REGISTER__ADDR_STACK(IR_Ins *ins, UNUSED void *dat, struct asm_procedure *code)
{
	...
	asm_reserve_register(ctx, dst->i, ins->dst);
	append_line(&code->body, fmt_str(
					"\tleaq %ld(%%rbp), %s\n",
					x->i,
					asm_reg_q_name[dst->i]));
}
...
emit_op_load_ADDR_REGISTER__ADDR_STACK(IR_Ins *ins, UNUSED void *dat, struct asm_procedure *code)
{
	...
	if (dst->tag == ADDR_ARGUMENT)
		asm_force_reserve_register(ctx, code, dst->i, ins->dst);
	else
		asm_reserve_register(ctx, dst->i, ins->dst);
	append_line(&code->body, fmt_str(
					"\tmov%c %ld(%%rbp), %s\n",
...
emit_op_load_ADDR_REGISTER__ADDR_REGISTER(IR_Ins *ins, UNUSED void *dat, struct asm_procedure *code)
{
	...
	asm_unassign_register(ctx, x->i);
	if (asm_register_assigned_p(ctx, dst->i) && dst->i != x->i) {
		struct asm_address *addr = &ctx->vars[ctx->assigned[dst->i]];
...
emit_op_load_ADDR_WIDE__ADDR_STACK(IR_Ins *ins, UNUSED void *dat, struct asm_procedure *code)
{
	...
	size_t ts = type_size(ins->type);
	if (ts == 16) {
		asm_reserve_register(ctx, dst->wide[0], ins->dst);
		asm_reserve_register(ctx, dst->wide[1], ins->dst);
		append_line(&code->body, fmt_str("\tmovq %ld(%%rbp), %s\n",
										 x->i, asm_reg_q_name[dst->wide[0]]));
		append_line(&code->body, fmt_str("\tmovq %ld(%%rbp), %s\n",
										 x->i + 8, asm_reg_q_name[dst->wide[1]]));
	} else {
		FAILWITH("TODO");
	}
}
...
emit_op_load_ADDR_WIDE__ADDR_REGISTER(IR_Ins *ins, UNUSED void *dat, struct asm_procedure *code)
{
	...
	if (ts == 16) {
		...
		if (dst->wide[0] == x->i || dst->wide[1] == x->i) {
			...
			append_line(&code->body, fmt_str("\tmovq (%s), %s\n",
											 asm_reg_q_name[tmp],
											 asm_reg_q_name[dst->wide[0]]));
			append_line(&code->body, fmt_str("\tmovq 8(%s), %s\n",
											 asm_reg_q_name[tmp],
											 asm_reg_q_name[dst->wide[1]]));
			asm_unassign_register(ctx, tmp);
		} else {
			append_line(&code->body, fmt_str("\tmovq (%s), %s\n",
											 asm_reg_q_name[x->i],
...
emit_op_load_ADDR_WIDE__GLOBL(IR_Ins *ins, void *dat, struct asm_procedure *code)
{
	...
	asm_force_reserve_register(ctx, code, dst->wide[0], ins->dst);
	asm_force_reserve_register(ctx, code, dst->wide[1], ins->dst);
...
emit_basic_op_ADDR_REGISTER__ADDR_STACK_LOAD__ADDR_IMM_INT(IR_Ins *ins, void *dat, struct asm_procedure *code)
{
	...
	assert(x->tag == ADDR_STACK_LOAD);
	assert(y->tag == ADDR_IMM_INT);
	int dst_reg = asm_reserve_register(ctx, dst->i, ins->dst);
...
asm_emit_div_mod(IR_Ins *ins, struct asm_procedure *code, bool remainder_p)
{
	...
	if (MATCH_ADDR3(ADDR_REGISTER, ADDR_STACK_LOAD, ADDR_IMM_INT)) {
		...
		dst->i = asm_assign_register(ctx, ins->dst);
				  ...
				  struct ir_toplevel *tl, struct asm_procedure *code)
{
	...
	if (MATCH_ADDR3(ADDR_REGISTER, ADDR_IMM_INT, ADDR_IMM_INT)) {
		enum asm_register dst_reg = dst->i;
		const char *dst_name = asm_reg_name(dst_reg, type);
		...
		struct asm_address *arg_addr = &ctx->vars[dst->i]; // lookup the addr for the formal block arg
		if (arg_addr->tag == ADDR_TEMP_REG) {
			arg_addr->tag = ADDR_REGISTER;
			arg_addr->i = asm_assign_callee_save_register(ctx, ins->dst);
		}
		...
		asm_unassign_register(ctx, x->i);
		dst->i = asm_reserve_register(ctx, x->i, ins->dst);
		dst->tag = ADDR_REGISTER;
		append_line(&code->body, fmt_str(
						"\t%s%c %d(%%rbp), %s\n",
						asm_op,
		...
		asm_unassign_register(ctx, x->i);
		int dst_reg = dst->i = asm_reserve_register(ctx, x->i, ins->dst);
		dst->tag = ADDR_REGISTER;
		const char *dst_name = asm_reg_name(dst_reg, type);
		const char *src_name = asm_source_operand_to_str(y, type, ctx, tl);
		append_line(&code->body, fmt_str(
		...
		x = &ctx->vars[x->i];
		int dst_reg = dst->i = asm_assign_register(ctx, ins->dst);
		dst->tag = ADDR_REGISTER;
		const char *dst_name = asm_reg_name(dst_reg, type);
		const char *src_name = asm_source_operand_to_str(y, type, ctx, tl);
		if (x->tag == ADDR_STACK) {
	...
	for (size_t i = 0; i < blk->code.len; ++i) {
		...
		case ir_op_copy: {
			...
			if (MATCH_ADDR2(ADDR_STACK, ADDR_REGISTER)) {
				emit_mem_cpy_code(code, "%rbp", dst->i, asm_reg_q_name[x->i], 0, ins->type);
				asm_unassign_register(ctx, x->i);
			} else {
		...
		case ir_op_cmpge: {
			...
			asm_unassign_register(ctx, dst.i);
		} break;
		...
		LBL_shift:
		{
			...
			struct asm_address *y   = &ctx->vars[ins->arg.rx[1]];
			if (MATCH_ADDR3(ADDR_REGISTER, ADDR_STACK_LOAD, ADDR_IMM_INT)) {
				const char *dst_name = asm_reg_name(dst->i, ins->type);
				int suffix = asm_suffix(ins->type);
				append_line(&code->body, fmt_str(
								"\tmov%c %d(%%rbp), %s\n",
								suffix,
		...
		case ir_op_neg: {
			...
			if (MATCH_ADDR2(ADDR_BLK_ARG, ADDR_STACK_LOAD)) {
				struct asm_address *arg_addr = &ctx->vars[dst->i];
				if (arg_addr->tag == ADDR_TEMP_REG) {
					arg_addr->tag = ADDR_REGISTER;
					arg_addr->i = asm_assign_callee_save_register(ctx, ins->dst);
				}
		...
		case ir_op_conslice: {
			...
			if (MATCH_ADDR3(ADDR_TEMP_WIDE, ADDR_REGISTER, ADDR_IMM_INT)) {
				dst->tag = ADDR_WIDE;
				dst->wide[0] = asm_assign_register(ctx, ins->dst);
				dst->wide[1] = asm_assign_register(ctx, ins->dst);
		...
		case ir_op_getelemptr: {
			...
			if (type_is_array_ptr(ins->type)) {
				...
				if (MATCH_ADDR3(ADDR_TEMP_REG, ADDR_STACK, ADDR_IMM_INT)) {
					...
					asm_unassign_register(ctx, base->i);
					dst->tag = ADDR_REGISTER;
					dst->i = asm_assign_register(ctx, ins->dst);
					append_line(&code->body, fmt_str(
									"\tleaq %ld(%s), %s\n",
									idx->i * scale,
									asm_reg_q_name[base->i],
									asm_reg_q_name[dst->i]));
				} else if (MATCH_ADDR3(ADDR_TEMP_REG, ADDR_REGISTER, ADDR_STACK_LOAD)) {
					assert(idx->type);
					dst->tag = ADDR_REGISTER;
					...
					append_line(&code->body, fmt_str(
									"\tmov%c %d(%%rbp), %s\n",
									asm_suffix(idx->type),
									idx->stack[0] + idx->stack[1],
									asm_reg_name(dst->i, idx->type)));
					...
					dst->tag = ADDR_REGISTER;
					dst->i = asm_assign_register(ctx, ins->dst);
					const char *dst_name = asm_reg_q_name[dst->i];
					append_line(&code->body, fmt_str(
									"\tmov%c %d(%%rbp), %s\n",
									asm_suffix(base->type),
									base->stack[0] + base->stack[1],
									asm_reg_name(dst->i, base->type)));
					append_line(&code->body, fmt_str(
									"\tleaq %ld(%s), %s\n",
									idx->i * scale,
									dst_name,
		...
		case ir_op_load: {
			...
			if (dst->tag == ADDR_STACK_LOAD) {
				...
				if (dst->tag == ADDR_TEMP_REG) {
					dst->tag = ADDR_REGISTER;
					dst->i = asm_assign_callee_save_register(ctx, ins->dst);
				} else {
					assert(dst->tag == ADDR_REGISTER);
					if (asm_register_assigned_p(ctx, dst->i)) {
						/* check if register was already assigned in another branch */
						struct asm_address *owner = &ctx->vars[asm_get_register_owner(ctx, dst->i)];
				...
				if (dst->tag == ADDR_STACK_LOAD) {
					...
					if (asm_register_assigned_p(ctx, dst->wide[0])) {
						/* check if register was already assigned in another branch */
						int owner_var = asm_get_register_owner(ctx, dst->wide[0]);
		...
		case ir_op_store: {
			...
			if (MATCH_ADDR2(ADDR_REGISTER, ADDR_IMM_INT)) {
				...
				append_line(&code->body, fmt_str(
								"\tmovq %s, (%s)\n",
								asm_reg_q_name[x->wide[0]],
								asm_reg_q_name[dst->i]));
				append_line(&code->body, fmt_str(
								"\tmovq %s, 8(%s)\n",
								asm_reg_q_name[x->wide[1]],
								asm_reg_q_name[dst->i]));
				asm_unassign_register(ctx, dst->i);
				asm_unassign_register(ctx, x->wide[0]);
				asm_unassign_register(ctx, x->wide[1]);
		...
		case ir_op_call: {
			...
			while (argc--) {
				...
				case ADDR_REGISTER:
				case ADDR_ARGUMENT:
					asm_unassign_register(ctx, arg->i);
					break;
				case ADDR_WIDE_ARG:
					asm_unassign_register(ctx, arg->wide[0]);
					asm_unassign_register(ctx, arg->wide[1]);
					break;
				case ADDR_PUSH_ARG:
			...
			if (!type_is_void(ins->type)) {
				switch ((int)ret->tag) {
				case ADDR_REGISTER: {
					asm_reserve_register(ctx, ret->i, ins->dst);
				} break;
				case ADDR_ARGUMENT: {
					...
					append_line(&code->body, fmt_str("\tmovq %s, %s\n",
													 ...
													 asm_reg_q_name[ret->i]));
					ret->extra.defered.fun = NULL;
				} break;
				case ADDR_WIDE_ARG: {
					asm_unassign_register(ctx, asm_arg_regs[0]);
					asm_unassign_register(ctx, asm_arg_regs[1]);
					asm_reserve_register(ctx, ret->wide[0], ins->dst);
					asm_reserve_register(ctx, ret->wide[1], ins->dst);
					append_line(&code->body, fmt_str("\tpushq %s\n", asm_reg_q_name[asm_ret_regs[0]]));
					append_line(&code->body, fmt_str("\tpushq %s\n", asm_reg_q_name[asm_ret_regs[1]]));
					append_line(&code->body, fmt_str("\tpopq %s\n",  asm_reg_q_name[ret->wide[1]]));
					append_line(&code->body, fmt_str("\tpopq %s\n",  asm_reg_q_name[ret->wide[0]]));
					ret->extra.defered.fun = NULL;
				} break;
				case ADDR_WIDE: {
					asm_reserve_register(ctx, ret->wide[0], ins->dst);
	...
	case ir_op_ret: {
		if (term->args.len > 0) {
			...
			if (ret->tag == ADDR_STACK) {
				...
				if (ret->wide[0] != (int)asm_ret_regs[0] || ret->wide[1] != (int)asm_ret_regs[1]) {
					append_line(&code->body, fmt_str("\tpushq %s\n", asm_reg_q_name[ret->wide[0]]));
					append_line(&code->body, fmt_str("\tpushq %s\n", asm_reg_q_name[ret->wide[1]]));
					append_line(&code->body, fmt_str("\tpopq %s\n",  asm_reg_q_name[asm_ret_regs[1]]));
